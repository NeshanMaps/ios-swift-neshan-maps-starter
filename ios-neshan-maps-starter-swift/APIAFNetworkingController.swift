//
//  APIAFNetworkingController.swift
...
class APIAFNetworkingController: UIViewController {
        ...
        let dataTask: URLSessionDataTask = manager.dataTask(with: request, uploadProgress: nil, downloadProgress: nil) { (response: URLResponse, responseObject: Any?, error: Error?) in
            if (error != nil || responseObject == nil) {
                ...
                if let responseObj = responseObject as? [String:Any] {
                    if responseObj["neighbourhood"] != nil {
                        self.addressTitle.text = responseObj["neighbourhood"] as? String
                    } else {
                        self.addressTitle.text = "آدرس نامشخص"
                    }
                    if responseObj["formatted_address"] != nil {
                        self.addressDetails.text = responseObj["formatted_address"] as? String
                    } else {
                        self.addressDetails.text = latLngAddr
                    }
                }
